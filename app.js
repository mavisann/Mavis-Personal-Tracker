...
(function () {
  ...
  var CATEGORY_DOT_CLASS = { Class: "dot-class", Event: "dot-event", Personal: "dot-personal" };
  ...
  function seedCourses() {
    return [
      { id: uid(), name: "Data Structures & Algorithms", code: "CS 202", professor: "Dr. A. Reyes", days: [1, 3], startTime: "09:00", endTime: "10:30", startDate: todayStr(), endDate: "2026-12-15", modality: "On-Site", room: "Rm 214", color: 0 },
      { id: uid(), name: "Linear Algebra", code: "MATH 210", professor: "Prof. L. Tan", days: [2, 4], startTime: "13:00", endTime: "14:30", startDate: todayStr(), endDate: "2026-12-15", modality: "Hybrid", room: "Rm 108", color: 1 },
      { id: uid(), name: "Technical Writing", code: "ENG 150", professor: "Ms. K. Cruz", days: [5], startTime: "10:00", endTime: "12:00", startDate: todayStr(), endDate: "2026-12-15", modality: "Online", room: "meet.school.edu/eng150", color: 2 }
    ];
  }
  function seedTasks() {
    var t = todayStr();
    var in2 = new Date(); in2.setDate(in2.getDate() + 2);
    var in5 = new Date(); in5.setDate(in5.getDate() + 5);
    function fmt(d) { return d.getFullYear() + "-" + pad2(d.getMonth() + 1) + "-" + pad2(d.getDate()); }
    return [
      { id: uid(), category: "Class", title: "Problem Set 4", courseName: "Data Structures & Algorithms", taskType: "Assignment", taskCode: "CS202-PS4", status: "In Progress", priority: "High", dueDate: fmt(in2), dueTime: "23:59", description: "Cover trees and graphs." },
      { id: uid(), category: "Class", title: "Midterm Exam", courseName: "Linear Algebra", taskType: "Exam", taskCode: "MATH210-MID", status: "To Do", priority: "Urgent", dueDate: fmt(in5), dueTime: "13:00", description: "Chapters 1-5." },
      { id: uid(), category: "Personal", title: "Renew student ID", location: "Registrar's Office", dueDate: t, dueTime: "15:00", priority: "Medium", status: "To Do", description: "" }
    ];
  }
  function seedTransactions() {
    var t = todayStr();
    return [
      { id: uid(), date: t, category: "Food", item: "Lunch", type: "Expense", amount: 120, method: "Cash" },
      { id: uid(), date: t, category: "Transport", item: "Jeepney fare", type: "Expense", amount: 25, method: "E-Wallet" },
      { id: uid(), date: t, category: "Income", item: "Allowance", type: "Income", amount: 5000, method: "Bank Transfer" },
      { id: uid(), date: t, category: "School Supplies", item: "Notebook set", type: "Expense", amount: 250, method: "Card" }
    ];
  }
  ...
  var ui = {
    ...
    editingBudget: false,
    miniCalCursor: { y: new Date().getFullYear(), m: new Date().getMonth(), d: new Date().getDate() },
  ...
  function buildSettingsRow() {
    ...
    return {
      id: "board",
      theme: s.theme, // Ensure theme is saved
  ...
  function renderShell() {
    ...
    html += '<aside class="sidebar ' + (ui.sidebarOpen ? '' : 'closed') + '" onmouseenter="App.openSidebar()" onmouseleave="App.closeSidebar()">' +
    ...
    html += '<nav class="bottom-nav">' + bottomNav + '</nav>';
  ...
  function renderMoneyMonitorCard() {
    ...
    return '<div class="card"><h3 class="card-title">Money monitor</h3>' +
      '<div class="metric" style="margin-bottom:12px;padding:16px"><p class="metric-label">On-hand</p><p class="metric-value" style="color:var(--teal)">' + fmtMoney(acc.onHand) + '</p></div>' +
      ...
      '<div class="metric" style="padding:16px"><p class="metric-label">Total available</p><p class="metric-value" style="color:var(--teal)">' + fmtMoney(currentBalance) + '</p></div>' +
      '</div>';
  }
  ...
  function renderDashboard() {
    ...
    var currentBalance = getCurrentBudget();
    var remaining = currentBalance;
    var pctUsed = currentBalance > 0 ? Math.min(100, (totalExpenses / Math.max(currentBalance, 1)) * 100) : 0;
    var low = pctUsed >= 80;
    ...
    html += '<div class="metric"><p class="metric-label">Total budget</p><p class="metric-value">' + fmtMoney(currentBalance) + '</p></div>';
    html += '<div class="metric"><p class="metric-label">Total expenses</p><p class="metric-value" style="color:var(--rose)">' + fmtMoney(totalExpenses) + '</p></div>';
    html += '<div class="metric">' +
      '<p class="metric-label">Remaining balance</p>' +
      '<p class="metric-value" style="color:' + (remaining < 0 ? "var(--rose)" : "var(--teal)") + '">' + fmtMoney(remaining) + '</p>' +
  ...
  function renderScheduleTimeline() {
    ...
    var blocks = todaysCourses.map(function (c) {
      ...
      var color = COURSE_COLORS[c.color % COURSE_COLORS.length];
      return '<div class="course-block" style="top:' + top + '%;height:' + height + '%;border-left-color:' + color + ';background:' + hexToRgba(color, 0.14) + ';color:' + color + '" title="' + escapeHtml(c.name) + ' | ' + fmtTime12(c.startTime) + ' - ' + fmtTime12(c.endTime) + '" onclick="App.openCourseModal(\'' + c.id + '\')" tabindex="0" onkeydown="if(event.key===\'Enter\'){App.openCourseModal(\'' + c.id + '\');}">' +
        ...
        '<p class="cb-sub">' + escapeHtml(c.professor) + ' &middot; ' + (c.modality === "Online" ? "Online" : escapeHtml(c.room)) + '</p>' +
        '<p class="cb-time">' + fmtTime12(c.startTime) + ' - ' + fmtTime12(c.endTime) + '</p></div>';
    }).join("");
    ...
    for (var d = 1; d <= daysInMonth; d++) {
      ...
      if (timelineCats.length) {
        dotHtml = '<span class="mini-cal-dots">' + timelineCats.slice(0, 3).map(function (cat) {
          return '<span style="background:' + (isToday ? "#fff" : CATEGORY_HEX[cat] || "#94a3b8") + '"></span>';
        }).join("") + '</span>';
      }
  ...
  function renderSchedule() {
    var header = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">' +
      '<h2 style="margin:0;font-size:18px">Weekly schedule</h2>' +
      '<button class="btn" onclick="App.openCourseModal()">' + icon("plus", 16) + ' Add course</button></div>';

    if (state.courses.length === 0) {
      return header + '<p style="color:var(--text-faint);font-size:13px">No courses yet. Add your first class.</p>';
    }
    ...
    for (var hour = 7; hour <= 22; hour++) {
      ...
      for (var day = 0; day < 7; day++) {
        ...
        var cellHtml = matches.length ? matches.map(function (course) {
          var color = COURSE_COLORS[course.color % COURSE_COLORS.length];
    ...
    return header + '<div class="card schedule-table-card" style="padding:0;overflow:auto"><table class="schedule-table"><thead><tr><th>Time</th>' + DAY_NAMES.map(function (d) { return '<th>' + d + '</th>'; }).join("") + '</tr></thead><tbody>' + hourRows.join("") + '</tbody></table></div>';
  }
  ...
  function renderBudget() {
    ...
    html += '<div class="metric"><p class="metric-label">Monthly income</p><p class="metric-value" style="color:var(--teal)">' + fmtMoney(monthIncome) + '</p></div>';
    html += '<div class="metric"><p class="metric-label">Monthly expenses</p><p class="metric-value" style="color:var(--rose)">' + fmtMoney(monthExpense) + '</p></div>';
    html += '<div class="metric"><p class="metric-label">Monthly balance</p><p class="metric-value" style="color:' + (monthIncome - monthExpense < 0 ? "var(--rose)" : "var(--text)") + '">' + fmtMoney(monthIncome - monthExpense) + '</p></div>';
    html += '<div class="metric"><p class="metric-label">Total budget</p><p class="metric-value" style="color:var(--teal)">' + fmtMoney(currentBalance) + '</p></div>';
    html += '</div>';
    ...
    html += '<div class="card"><h3 class="card-title">Money monitor</h3>' +
      '<div class="metric" style="margin-bottom:12px;padding:16px"><p class="metric-label">On-hand</p><p class="metric-value" style="color:var(--teal)">' + fmtMoney(accountSnapshot.onHand) + '</p></div>' +
    ...
    html += '<div class="card"><h3 class="card-title">Quick summary</h3>' +
      '<div style="display:grid;gap:12px"><div class="metric" style="padding:16px"><p class="metric-label">Current balance</p><p class="metric-value" style="color:var(--teal)">' + fmtMoney(currentBalance) + '</p></div>' +
      '<div class="metric" style="padding:16px"><p class="metric-label">Top category</p><p class="metric-value" style="color:var(--rose)">' + (topCategory ? escapeHtml(topCategory.name) : "—") + '</p></div></div>' +
      '</div>';
    html += '</div>';
  ...
  function renderLineChart(monthlyFlow) {
    ...
    var incomePts = monthlyFlow.map(function (m, i) { return pt(i, m.income); }).join(" ");
    var expensePts = monthlyFlow.map(function (m, i) { return pt(i, m.expense); }).join(" ");
    var gridLines = "";
    for (var g = 0; g <= 3; g++) {
    ...
    return '<svg class="linechart" viewBox="0 0 ' + w + ' ' + h + '" preserveAspectRatio="none">' +
      gridLines + labels +
      '<polyline points="' + incomePts + '" fill="none" stroke="#0d9488" stroke-width="2"/>' +
      '<polyline points="' + expensePts + '" fill="none" stroke="#e11d48" stroke-width="2"/>' +
      '</svg>' +
      '<div class="linechart-legend"><span><i style="background:#0d9488"></i> Income</span><span><i style="background:#e11d48"></i> Expense</span></div>';
  }
  ...
  function taskCardHtml(task) {
    ...
    var html = '<div class="task-card"><div class="task-card-top"><div style="min-width:0">' +
      '<p class="task-card-title">' + escapeHtml(task.title) + '</p>' +
      '<p class="task-card-sub">' + sub + '</p></div>' +
      '<div style="display:flex;gap:8px;align-items:center"><button class="icon-btn" title="Edit task" onclick="App.openTaskModal(\'' + task.id + '\');event.stopPropagation();">' + icon("pencil", 13) + '</button><button class="icon-trash" onclick="App.deleteTask(\'' + task.id + '\');event.stopPropagation();">' + icon("trash", 13) + '</button></div></div>' +
  ...
  function renderTasks() {
    ...
    var html = '<div class="toolbar">' +
      '<h2 style="margin:0;font-size:18px">Tasks</h2>' +
      '<div class="toolbar-right">' +
    ...
    if (ui.taskView === "kanban") {
      ...
      STATUSES.forEach(function (col) {
        ...
        html += '<div class="kanban-col"><p class="kanban-col-title">' + col + '</p><div>';
    ...
    if (tasks.length === 0) {
      ...
      tasks.forEach(function (task) {
        ...
        html += '<tr>' +
          ...
          '<td style="text-align:right"><button class="icon-trash" onclick="App.deleteTask(\'' + task.id + '\')">' + icon("trash", 14) + '</button></td>' +
          '</tr>';
      });
    }
  ...
  function renderTaskCalendar(tasks) {
    ...
    for (var d = 1; d <= daysInMonth; d++) {
      ...
      var pills = dayTasks.slice(0, 4).map(function (t) {
        var color = CATEGORY_HEX[t.category] || "#64748b";
        var done = t.status === "Completed";
        return '<button class="task-pill ' + (done ? "done" : "") + '" style="background:' + (done ? "" : color) + '" title="Open task" onclick="App.openTaskModal(\'' + t.id + '\')">' + escapeHtml(t.title) + '<br/><span style="opacity:.8">' + (t.dueTime || "Any time") + '</span></button>';
      }).join("");
  ...
  function renderMonthlyCalendar() {
    ...
    function dayItems(d) {
      ...
      var dow = new Date(cur.y, cur.m, d).getDay();
      var items = [];
      if (ui.genCalShow.Class) state.courses.forEach(function (c) { if (c.days.indexOf(dow) !== -1) items.push({ type: "Class", label: c.code }); });
      state.tasks.forEach(function (t) { if (t.dueDate === key && ui.genCalShow[t.category]) items.push({ type: t.category, label: t.title }); });
      return items;
    }
    ...
    for (var d = 1; d <= daysInMonth; d++) {
      ...
      var itemsHtml = items.slice(0, 3).map(function (it) {
        return '<div class="gen-item" style="background:' + (CATEGORY_HEX[it.type] || "#64748b") + '">' + escapeHtml(it.label) + '</div>';
      }).join("");
      var more = items.length > 3 ? '<p style="font-size:9px;color:var(--text-faint);margin:0">+' + (items.length - 3) + ' more</p>' : "";
      cells += '<div class="gen-cal-day ' + (isToday ? "today" : "") + '" onclick="App.openDayDetail(\'' + dayKey + '\')" style="cursor:pointer"><p class="gen-cal-num" style="' + (isToday ? "color:var(--teal-dark)" : "") + '">' + d + '</p>' + itemsHtml + more + '</div>';
    }
    var dow = DAY_NAMES.map(function (d) { return '<div class="mini-cal-dow">' + d + '</div>'; }).join("");
  ...
  function renderWeeklyCalendar(date) {
    ...
    for (var day = 0; day < 7; day++) {
      ...
      var cellHtml = allDayItems.map(function (task) {
        var color = CATEGORY_HEX[task.category] || "#64748b";
      ...
      for (var day = 0; day < 7; day++) {
        ...
        var cellHtml = matches.map(function (item) {
          ...
          if (item.startTime) {
            ...
            title = escapeHtml(item.name); code = escapeHtml(item.code); name = escapeHtml(item.name); start = fmtTime12(item.startTime); end = fmtTime12(item.endTime);
            return '<div class="schedule-chip" style="background:' + hexToRgba(color, 0.12) + '; border-left: 3px solid ' + color + '; color:' + color + ';" onclick="' + onclick + '" title="' + title + '"><strong>' + code + '</strong><span>' + name + '</span><small>' + start + '-' + end + '</small></div>';
          } else {
            color = CATEGORY_HEX[item.category] || "#64748b";
  ...
  function renderGeneralCalendar() {
    ...
    var toggles = ["Class", "Event", "Personal"].map(function (cat) {
      return '<button class="cal-toggle ' + (ui.genCalShow[cat] ? "active" : "") + '" onclick="App.toggleGenCalShow(\'' + cat + '\')"><span style="background:' + CATEGORY_HEX[cat] + '"></span>' + cat + '</button>';
    }).join("");
  ...
  function renderDayDetailList(dateKey) {
    ...
    state.courses.forEach(function (course) { if (course.days.indexOf(dow) !== -1) { items.push({ kind: "course", id: course.id, title: course.name, label: course.code, time: fmtTime12(course.startTime) + " - " + fmtTime12(course.endTime), meta: course.modality + " • " + (course.modality === "Online" ? course.room : course.room || "Room TBD"), color: COURSE_COLORS[course.color % COURSE_COLORS.length], sortMinutes: minutesOf(course.startTime) }); } });
    state.tasks.filter(function (t) { return t.dueDate === dateKey; }).forEach(function (t) {
      items.push({
        ...
        color: t.priority === "Urgent" ? "#e11d48" : (t.priority === "High" ? "#d97706" : (t.priority === "Medium" ? "#0284c7" : "#0d9488")),
    ...
    return '<div class="day-detail-list">' + items.map(function (item) {
      return '<div class="day-detail-item" style="border-left-color:' + escapeHtml(item.color) + ';background:' + hexToRgba(item.color, 0.08) + '">' +
        ...
        '<div class="day-detail-header"><p class="day-detail-title">' + escapeHtml(item.title) + '</p><button class="btn btn-sm" onclick="App.' + (item.kind === "course" ? "openCourseModal('" + item.id + "')" : "openTaskModal('" + item.id + "')") + '">Edit</button></div>' +
  ...
  function renderCourseForm() {
    ...
    var colorDots = COURSE_COLORS.map(function (c, i) {
      return '<button type="button" class="color-dot ' + (d.color === i ? "active" : "") + '" style="background:' + c + '" onclick="App.setCourseColor(' + i + ')"></button>'; // Corrected color dot rendering
    }).join("");
    ...
    return '<form onsubmit="return App.submitCourseForm(event)">' +
      '<div class="row-2">' +
      ...
      '</div>' +
      '<div class="field"><span class="field-label">Color</span><div class="chip-row">' + colorDots + '</div></div>' +
      ...
      (ui.modal.payload ? '<button type="button" class="btn-danger" style="margin-right: auto; background-color: var(--rose); border-color: var(--rose);" onclick="App.deleteCourseWithConfirmation(\'' + ui.modal.payload.id + '\')">' + icon("trash", 14) + ' Delete</button>' : '') +
  ...
  function renderTaskForm() {
    ...
    var recurrenceRadios = repeatOptions.map(function(type) {
        return '<label class="radio-label-pill">' +
            '<input type="radio" name="taskRecurrence" value="' + type + '" onchange="App.updateDraft(\'taskRepeatType\', this.value); App.render();" ' + (d.taskRepeatType === type ? 'checked' : '') + '>' +
    ...
    return '<form onsubmit="return App.submitTaskForm(event)" autocomplete="off">' +
      ...
      '<div class="modal-actions"><button type="button" class="btn-ghost" onclick="App.closeModal()">Cancel</button><button type="submit" class="btn">Save item</button></div></form>';
   }
  ...
  var App = {
    ...
    toggleGenCalShow: function (cat) { ui.genCalShow[cat] = !ui.genCalShow[cat]; render(); },
    ...
    openCourseModal: function (id) {
      ...
      ui.formDraft = course ? Object.assign({}, course, { courseRecurrenceType: courseRecurrenceType }) : {
        ...
        endDate: "", modality: "On-Site", room: "", color: 0, courseRecurrenceType: 'none'
      };
    ...
    setCourseRecurrenceType: function (type) {
      ...
      render(); // Re-render to update day chips visibility
    },
    setCourseColor: function (i) { ui.formDraft.color = i; render(); },
    setCourseModality: function (val) { ui.formDraft.modality = val; render(); },
    submitCourseForm: function (e) {
      ...
      if (existingIndex !== -1) {
        state.courses[existingIndex] = Object.assign({}, d, { id: ui.modal.payload.id });
      } else {
        var newCourse = Object.assign({}, d, { id: uid() });
        state.courses.push(newCourse);
        showToast("Course '" + newCourse.name + "' added successfully!", "success");
      }
      ui.modal = null;
      commit();
      return false;
    ...
    setTheme: function (value) {
      state.settings.theme = value || "light";
      commit().then(function() {
    ...
    addBudgetCategory: function () {
      var input = document.getElementById("budget-category-input");
      var value = input ? input.value.trim() : "";
      if (!value) return;
      var categories = getBudgetCategories();
      if (categories.indexOf(value) === -1) {
        categories.push(value);
        state.settings.budgetCategories = categories;
        commit().then(function() {
          showToast("Budget category '" + value + "' added.", "success");
        });
      }
      if (input) input.value = "";
    },
    ...
    setTaskFormCategory: function (cat) { ui.formDraft.category = cat; render(); },
